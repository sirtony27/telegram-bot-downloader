...
export async function handleDownload(ctx, url) {
  ...
  try {
    ...
    logger.info(
      `[downloadHandler] Archivo descargado: ${filePath} (${fileSizeMb} MB)`,
    );
    ...
    const isAudio = [".mp3", ".m4a", ".ogg", ".opus", ".flac", ".wav"].includes(
      ext,
    );
    ...
    if (err.message.includes("timeout") || err.message.includes("Timeout")) {
      userMessage =
        "⏱ La descarga tardó demasiado y fue cancelada. Intentá con otro video.";
    } else if (
      err.message.includes("max-filesize") ||
      err.message.includes("File is too large")
    ) {
      ...
      userMessage =
        "🔐 Este sitio requiere autenticación para descargar desde servidores. Configurá el archivo de cookies (COOKIES_FILE en .env).";
    } else if (err.message.includes("yt-dlp falló")) {
      userMessage =
        "❌ No se pudo descargar el video. Verificá que la URL sea válida y que el contenido sea público.";
    ...
    if (filePath) {
      unlink(filePath).catch((e) =>
        logger.warn(
          `[downloadHandler] No se pudo eliminar el temporal ${filePath}:`,
          e.message,
        ),
      );
    }
